...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < times.length; i++) {
    var name = names[i];
    var height = step * (time - 0);

    ctx.fillText(parseInt(time, 10), initialX + (histogramWidth + indent) * i, 90 + histogramHeight - height);
    ctx.fillText(name, initialX + (histogramWidth + indent) * i, 100 + histogramHeigh + 20);
